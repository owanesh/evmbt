...
struct hera_instance : evmc_instance {
  ...
  hera_instance() noexcept : evmc_instance({EVMC_ABI_VERSION, "hera", "0.0.1", nullptr, nullptr, nullptr, nullptr, nullptr}) {}
};
...
static struct evmc_result hera_execute(struct evmc_instance* vm,
                                  ...
                                  size_t code_size)
{
  ...
  try {
    ...
    // Output WASM code to file
    // std::ofstream myfile;
    // myfile.open("/home/toor/evmTrans/blockchain/wasm_test.wasm", std::ios::out | std::ios::binary | std::ios::trunc);
...
static void hera_destroy (evmc_instance* vm)
{
  hera_instance* hera = static_cast<hera_instance*>(vm);
  delete hera;
}

evmc_capabilities_flagset hera_get_capabilities(evmc_instance* vm)
{
...
void hera_set_tracer(struct evmc_instance* instance, evmc_trace_callback callback, struct evmc_tracer_context* context) {
}
...
evmc_instance* evmc_create_hera() noexcept
{
  ...
  instance->set_tracer = hera_set_tracer;
