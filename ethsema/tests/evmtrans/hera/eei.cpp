...
// #endif
  ...
  void EthereumInterface::eeiCallDataCopy(uint32_t resultOffset, uint32_t dataOffset, uint32_t length)
  {
      ...
      std::cerr << "eeiCALLDATACOPY -> calldata size=0x" << m_msg.input_size << "\n";
      for (int i = 0; i < m_msg.input_size; ++i){
         std::cerr << std::setfill('0') << std::setw(2) << hex << (int)(*(m_msg.input_data+i)) << "";
      }
      // std::cerr << "\n" << "write to memory 0x" << length << " Bytes\n" << " dataOffset, resultOffset= " << dataOffset << ", "<< resultOffset << "\n";
      // storeMemory({m_msg.input_data, m_msg.input_size}, dataOffset, resultOffset, length);
      // std::cerr << "End strore Memory\n" ;

      std::cerr << "\ndataOffset:" << (int)dataOffset << " ; length: " << (int)length << "\n";
      std::cerr << "msg.data_size :" << (int)m_msg.input_size;
      ...
      { 
        ...
        std::cerr <<  " write2 @" << m_msg.input_size-dataOffset << "\n";
        storeMemory({m_msg.input_data, m_msg.input_size}, dataOffset, resultOffset, m_msg.input_size-dataOffset);
        std::cerr << "End strore Memory\n" ;
      }
  ...
  {   
      // HERA_DEBUG << depthToString() << " msg.value= " << static_cast<uint32_t>(m_msg.value) << dec << "\n";
  ...
  void EthereumInterface::eeiCodeCopy(uint32_t resultOffset, uint32_t codeOffset, uint32_t length)
  {
      ...
      std::cerr << "eeiCodeCopy -> code =\n0x";
      // for (int i = 0; i < static_cast<uint32_t>(m_code.size()); ++i){
      //   //  std::cerr << std::setfill('0') << std::setw(2) << hex << (int)(m_code[i]) << "";
      //   std::cerr << m_code[i] << "";
      // } 
      std::cerr << "\n size=" << static_cast<uint32_t>(m_code.size());
      std::cerr << "\neeiCodeCopy " << "-> 2memory @(codeOffset, length) = " << codeOffset << length;

      // std::cerr << "Another Way:\n";
  ...
  void EthereumInterface::eeiExternalCodeCopy(uint32_t addressOffset, uint32_t resultOffset, uint32_t codeOffset, uint32_t length)
  {
      ...
      storeMemory(codeBuffer, 0, resultOffset, length); // store the entire bytecode
  }
  ...
  {   
      std::cerr << depthToString() << " storageStore " << hex << pathOffset << " " << valueOffset << dec << "\n";
      ...
      std::cerr << "SSTORE@path2storage -> ";
      for (int i = 0; i < sizeof(path.bytes) / sizeof(path.bytes[0]); ++i){
         std::cerr << std::setfill('0') << std::setw(2) << hex << (int)path.bytes[i];
      }
      std::cerr << "\n";
      std::cerr << "SSTORE@value2storage -> ";
      for (int i = 0; i < sizeof(value.bytes) / sizeof(value.bytes[0]) ; ++i){
         std::cerr << std::setfill('0') << std::setw(2) << hex << (int)value.bytes[i];
      }
      ...
      std::cerr << "SSTORE@going to update storage\n";
      ...
      std::cerr << "SSTORE@ updated\n";
  }

  void EthereumInterface::eeiStorageLoad(uint32_t pathOffset, uint32_t resultOffset)
  {
      std::cerr << depthToString() << " storageLoad " << hex << pathOffset << " " << resultOffset << dec << "\n";
      ...
      std::cerr << "SLOAD@memory path: ";
      for (int i = 0; i < sizeof(path.bytes) / sizeof(path.bytes[0]); i++){
         std::cerr << std::setfill('0') << std::setw(2) << hex << (int)path.bytes[i];
      }
      std::cerr << "\n";
      std::cerr << "SLOAD@storage result: ";
      for (int i = 0; i < sizeof(result.bytes) / sizeof(result.bytes[0]); i++){
         std::cerr << std::setfill('0') << std::setw(2) << hex << (int)result.bytes[i];
      }
...
// #endif
      ...
      std::cerr << "Call-Host Exec-> Status=" << call_result.status_code << "\n";
      ...
      case EVMC_SUCCESS:
        // std::cerr << "callres= @ ";
        // for (int i = 0; i < call_result.output_size; ++i){
        //   std::cerr << std::setfill('0') << std::setw(2) << hex << (int)(*(call_result.output_data+i));
        // }
        // std::cerr << " @\n";
        
        // std::cerr << "m_lastReturn @\n";
        // for (int i = 0; i <  i < m_lastReturnData.size(); ++i){
        //   std::cerr << std::setfill('0') << std::setw(2) << hex << (int)m_lastReturnData[i];
        // }
        // std::cerr << " @\n";
        // std::cerr << "End CALL\n";
        return 0;
  ...
  { 
    // std::cerr <<"*"<< "destOffset=" << dstOffset << " length=" <<  length << " memsize = " << memorySize() <<"\n";
    ensureCondition((dstOffset + length) >= dstOffset, InvalidMemoryAccess, "Out of bounds (destination) memory copy.");
    ensureCondition(memorySize() >= (dstOffset + length), InvalidMemoryAccess, "Out of bounds (destination) memory copy.");
  ...
  {
    // std::cerr << "sstoreMeory-@ destOffset=" << dstOffset << " srcOffset=" << srcOffset << " length=" <<  length << " memsize = " << memorySize() <<"\n";
    ensureCondition((srcOffset + length) >= srcOffset, InvalidMemoryAccess, "Out of bounds (source) memory copy.");
