...
std::string makeCodeId(evmc_address destination)
{
...
unique_ptr<WasmEngine> WabtEngine::create2()
{
  return unique_ptr<WasmEngine>{new WabtEngine};
}
...
ExecutionResult WabtEngine::execute(
  ...
  hostModule->AppendFuncExport(
    ...
    [&interface](
      ...
      results[0].set_i32(interface.eeiCreate2(
        args[0].value.i32, args[1].value.i32,
        args[2].value.i32, args[3].value.i32, 
        args[4].value.i32
      ));
  ...
  for (auto it = errors.begin(); it != errors.end(); ++it) {
    std::cerr << "wabt (execute): " << it->message << "\n";
  }

  // std::cerr << "wabt: before ensureCondition.\n";
  ...
  // std::cerr << "wabt: ensureCondition passed\n";

  // Prepare to execute
  auto codeIdentifier = makeCodeId(msg.destination);
  // std::cerr << "WABT: looking for export function main\n";
  interp::Export* mainFunction = module->GetExport("main");
  // interp::Export* mainFunction = module->GetExport(codeIdentifier);
  // std::cerr << "WABT: engine.execute " << codeIdentifier << "\n";
