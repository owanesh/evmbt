#pragma once

#include "CompilerHelper.h"

namespace dev
{
namespace eth
{
namespace trans
{

class Arith256
{
public:
	Arith256(IRBuilder& _builder, llvm::Module *_module);

	llvm::Value* exp(llvm::Value* _arg1, llvm::Value* _arg2);

	static void debug(llvm::Value *_value, char _c, llvm::Module &_module, IRBuilder &_builder);

	static llvm::Function* getUDiv256Func(llvm::Module& _module);
	static llvm::Function* getURem256Func(llvm::Module& _module);
	static llvm::Function* getURem512Func(llvm::Module& _module);
	static llvm::Function* getUDivRem256Func(llvm::Module& _module);
	static llvm::Function* getSDiv256Func(llvm::Module& _module);
	static llvm::Function* getSRem256Func(llvm::Module& _module);
	static llvm::Function* getSDivRem256Func(llvm::Module& _module);
	static llvm::Function* getUDivRem512Func(llvm::Module& _module);
	static llvm::Function* getMulFunc(llvm::Module& _module, uint64_t BitWidth, const std::string funcName);
	static llvm::Function* getMul256Func(llvm::Module& _module);
	static llvm::Function* getMul128Func(llvm::Module& _module);


	// safe math
	static llvm::Function* getSafeMathAddFunc(llvm::Module& _module);
	static llvm::Function* getSafeMathSubFunc(llvm::Module& _module);
	static llvm::Function* getSafeMathMulFunc(llvm::Module& _module);
	static llvm::Function* getSafeMathSDivFunc(llvm::Module& _module);
	static llvm::Function* getSafeMathUDivFunc(llvm::Module& _module);

private:
	llvm::Function* getExpFunc();

	llvm::Function* m_exp = nullptr;
	IRBuilder& m_builder;
	llvm::Module* m_module = nullptr;
};


}
}
}
