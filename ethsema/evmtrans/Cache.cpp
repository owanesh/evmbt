...
namespace
{
	...
	std::string getVersionedCacheDir()
	{
		...
		// llvm::sys::path::user_cache_directory(path, "ethereum", "evmtrans",
		//                                       std::to_string(c_internalABIVersion));
		llvm::sys::path::append(path, "ethereum", "evmtrans",
		                                      std::to_string(c_internalABIVersion));
		return path.str();
	}
...
std::unique_ptr<llvm::Module> Cache::getObject(std::string const& id, llvm::LLVMContext& _llvmContext)
{
	...
	{
		// DLOG(cache) << id << ": found\n";
		auto module = std::make_unique<llvm::Module>(id, _llvmContext);
